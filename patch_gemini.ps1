...
<#
    @{
        ...
        Match = '(?s)async function getProcessTableWindows\(\) \{.*?return processMap;\s*\}(?=\s*async function)'
        ReplaceTarget = '(?s)async function getProcessTableWindows\(\) \{.*?return processMap;\s*\}(?=\s*async function)'
...
'@
    $patchedLoadingGuard = Update-PatchRulesToText $loadingGuardFixture
    Assert-SelfTest "slash command loading guard is inserted" ($patchedLoadingGuard -match 'Slash commands are still loading')
    Assert-SelfTest "slash command loading guard is idempotent" ((Update-PatchRulesToText $patchedLoadingGuard) -eq $patchedLoadingGuard)
...
if (!commandToExecute) {
      ...
      setIsProcessing(true);
'@
    $patchedUnknownGuard = Update-PatchRulesToText $unknownGuardFixture
    ...
    Assert-SelfTest "unknown slash command guard is idempotent" ((Update-PatchRulesToText $patchedUnknownGuard) -eq $patchedUnknownGuard)
...
'@
    $patchedRetryDuplicate = Update-PatchRulesToText $retryDuplicateFixture
    ...
    Assert-SelfTest "retry UI duplicate cleanup is idempotent" ((Update-PatchRulesToText $patchedRetryDuplicate) -eq $patchedRetryDuplicate)
    ...
    $patchedZombieDuplicate = Update-PatchRulesToText $zombieDuplicateFixture
    ...
    Assert-SelfTest "zombie process patch is idempotent" ((Update-PatchRulesToText $patchedZombieDuplicate) -eq $patchedZombieDuplicate)
